...
if ( !class_exists('BackuplyUpdateChecker_3_2', false) ):
 ...
 * @access public
 */
class BackuplyUpdateChecker_3_2 {
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
		...
		$this->upgraderStatus = new Backuply_PucUpgraderStatus_3_2();
	 ...
	 * @return Backuply_PucScheduler_3_2
	 */
	protected function createScheduler($checkPeriod) {
		return new Backuply_PucScheduler_3_2($this, $checkPeriod);
	}
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($status) ){
			$pluginInfo = BackuplyInfo_3_2::fromJson($result['body']);
			if ( $pluginInfo !== null ) {
	 ...
	 *
	 * @uses BackuplyUpdateChecker::requestInfo()
	 *
	 * @return BackuplyUpdate_3_2 An instance of BackuplyUpdate, or NULL when no updates are available.
	 */
	public function requestUpdate(){
		...
		$update = BackuplyUpdate_3_2::fromBackuplyInfo($pluginInfo);
	 ...
	 *
	 * @return BackuplyUpdate_3_2|null
	 */
	...
	public function getUpdateState() {
		...
		if ( isset($state, $state->update) && is_object($state->update) ) {
			$state->update = BackuplyUpdate_3_2::fromObject($state->update);
		}
		return $state;
	}
	...
	private function setUpdateState($state) {
		if ( isset($state->update) && is_object($state->update) && method_exists($state->update, 'toStdClass') ) {
			$update = $state->update; /** @var BackuplyUpdate_3_2 $update */
	 ...
	 * @param BackuplyUpdate_3_2 $updateToAdd
	 * @return StdClass
	 */
	 ...
	 * successfully, the first argument passed will be an instance of  BackuplyInfo. Otherwise, 
	 * it will be NULL. The second argument will be the corresponding return value of 
	 * wp_remote_get (see WP docs for details).
	 *  
	 * The callback function should return a new or modified instance of BackuplyInfo or NULL.
	 * 
	 * @uses add_filter() This method is a convenience wrapper for add_filter().
	 * 
	 * @param callable $callback
	...
	public function initDebugBarPanel() {
		...
		if ( class_exists('Debug_Bar', false) && file_exists($debugBarPlugin) ) {
			...
			$this->debugBarPlugin = new Backuply_PucDebugBarPlugin_3_2($this);
		}
	}
...
if ( !class_exists('BackuplyInfo_3_2', false) ):
...
class BackuplyInfo_3_2 {
	...
	/**
	 * Create a new instance of BackuplyInfo from JSON-encoded plugin info 
	 * returned by an external update API.
	 * 
	 * @param string $json Valid JSON string representing plugin info.
	 * @return BackuplyInfo_3_2|null New instance of BackuplyInfo, or NULL on error.
	 */
...
if ( !class_exists('BackuplyUpdate_3_2', false) ):
...
class BackuplyUpdate_3_2 {
	...
	/**
	 * Create a new instance of BackuplyUpdate from its JSON-encoded representation.
	 * 
	 * @param string $json
	 * @return BackuplyUpdate_3_2|null
	 */
	public static function fromJson($json){
		...
		$pluginInfo = BackuplyInfo_3_2::fromJson($json);
		if ( $pluginInfo != null ) {
			return self::fromBackuplyInfo($pluginInfo);
		} else {
			return null;
		}
	}
	...
	/**
	 * Create a new instance of BackuplyUpdate based on an instance of BackuplyInfo.
	 * Basically, this just copies a subset of fields from one object to another.
	 * 
	 * @param BackuplyInfo_3_2 $info
	 * @return BackuplyUpdate_3_2
	 */
	public static function fromBackuplyInfo($info){
		return self::fromObject($info);
	}
	
	/**
	 * Create a new instance of BackuplyUpdate by copying the necessary fields from 
	 * another object.
	 *  
	 * @param StdClass|BackuplyInfo_3_2|BackuplyUpdate_3_2 $object The source object.
	 * @return BackuplyUpdate_3_2 The new copy.
	 */
	 ...
	 * a BackuplyUpdate. Useful for serialization and caching, as it avoids
...
if ( !class_exists('Backuply_PucScheduler_3_2', false) ):

/**
 * The scheduler decides when and how often to check for updates.
 * It calls @see BackuplyUpdateChecker::checkForUpdates() to perform the actual checks.
 *
 * @version 3.2
 */
class Backuply_PucScheduler_3_2 {
	public $checkPeriod = 12; //How often to check for updates (in hours).
	public $throttleRedundantChecks = false; //Check less often if we already know that an update is available.
	public $throttledCheckPeriod = 72;

	/**
	 * @var BackuplyUpdateChecker_3_2
	 */
	 ...
	 *
	 * @param BackuplyUpdateChecker_3_2 $updateChecker
...
if ( !class_exists('Backuply_PucUpgraderStatus_3_2', false) ):
 ...
 */
class Backuply_PucUpgraderStatus_3_2 {
...
if ( !class_exists('Backuply_PucFactory', false) ):

/**
 * A factory that builds instances of other classes from this library.
 *
 * When multiple versions of the same class have been loaded (e.g. BackuplyUpdateChecker 1.2
 * and 1.3), this factory will always use the latest available version. Register class
 * versions by calling {@link Backuply_PucFactory::addVersion()}.
 *
 * At the moment it can only build instances of the BackuplyUpdateChecker class. Other classes
 * are intended mainly for internal use and refer directly to specific implementations. If you
 * want to instantiate one of them anyway, you can use {@link Backuply_PucFactory::getLatestClassVersion()}
 * to get the class name and then create it with <code>new $class(...)</code>.
 */
class Backuply_PucFactory {
	protected static $classVersions = array();
	protected static $sorted = false;

	/**
	 * Create a new instance of BackuplyUpdateChecker.
	 *
	 * @see BackuplyUpdateChecker::__construct()
	 *
	 * @param $metadataUrl
	 * @param $pluginFile
	 * @param string $slug
	 ...
	 *
	 * @param string $generalClass Class name without version numbers, e.g. 'BackuplyUpdateChecker'.
	 * @param string $versionedClass Actual class name, e.g. 'BackuplyUpdateChecker_1_2'.
...
Backuply_PucFactory::addVersion('BackuplyUpdateChecker', 'BackuplyUpdateChecker_3_2', '3.2');
Backuply_PucFactory::addVersion('BackuplyUpdate', 'BackuplyUpdate_3_2', '3.2');
Backuply_PucFactory::addVersion('BackuplyInfo', 'BackuplyInfo_3_2', '3.2');
Backuply_PucFactory::addVersion('Backuply_PucGitHubChecker', 'Backuply_PucGitHubChecker_3_2', '3.2');
